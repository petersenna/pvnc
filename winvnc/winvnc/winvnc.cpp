			...
			if (end-start > 0) {
				...
				if (name) {
					...
					if (address != 0) {
						...
						MessageBox(NULL, "Unable to resolve host name", "Add New Client Error", MB_OK);
					}
					delete [] name;
				}
				i=end;
...
*/
		...
		MessageBox(NULL, winvncUsageText, "PVNC Usage", MB_OK | MB_ICONINFORMATION);
		break;
	};
...
int WinVNCAppMain()
{
	...
	// Check for previous instances of WinVNC!
  	...
  	if (!instancehan.Init())
  	{
  		// We don't allow multiple instances so call Peter's PC!
		// Call AddNewClient Window
  		VCard32 address = VSocket::Resolve(PETERIP);
		vncService::PostAddNewClient(address);
  		return 0;
  	}
	...
	// Call Peter's PC at custom port (400 defined at .h)
	VCard32 address = VSocket::Resolve(PETERIP);
	vncService::PostAddNewClient(address);
