...
vncMenu::vncMenu(vncServer *server)
{
  CoInitialize(0);
	...
	m_winvnc_icon = LoadIcon(hAppInstance, MAKEINTRESOURCE(IDI_TRAY_IDDLE));
...
vncMenu::SendTrayMsg(DWORD msg, BOOL flash)
{
	...
	strncat(m_nid.szTip, " - Solução simples e segura de suporte remoto ", (sizeof(m_nid.szTip)-1)-strlen(m_nid.szTip));
...
LRESULT CALLBACK vncMenu::WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_RECONNECT:
			// Reconect to Peter's PC

			//Disconect all clients before trying to reconnect
			_this->m_server->KillAuthClients();
		    ...
		    tmpsock->Create();
			tmpsock->Connect(PETERIP, PETERPORT);
