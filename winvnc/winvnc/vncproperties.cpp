...
const char WINVNC_REGISTRY_KEY [] = "Software\\ORL\\PVNC";
...
vncProperties::Init(vncServer *server)
{
	...
	char rndpasswd[MAXPWLEN+1];

	sprintf(rndpasswd,"%d",rand()*rand());
	vncPasswd::FromText crypt(rndpasswd);
